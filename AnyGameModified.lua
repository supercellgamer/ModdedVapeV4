						...
						spawn(function()
							...
							repeat
								if entity.isAlive then 
									...
									newpos = newpos == newpos and newpos * (math.clamp((lplr.Character.HumanoidRootPart.CFrame.p - selectedpos).magnitude, 0, ClickTPAmount["Value"])) or Vector3.new(0, 0, 0)
									lplr.Character.HumanoidRootPart.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(newpos.X, (ClickTPVertical["Enabled"] and newpos.Y or 0), newpos.Z)
									lplr.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
							...
							until entity.isAlive and (lplr.Character.HumanoidRootPart.CFrame.p - selectedpos).magnitude <= 5 or (not ClickTP["Enabled"])
					...
					if tool and (killauramouse["Enabled"] and uis:IsMouseButtonPressed(0) or (not killauramouse["Enabled"])) then
						...
						if touch then
							...
							for i,v in pairs(workspace.Zombies.Spawned:GetChildren()) do
                                for i,v in pairs(workspace.BossArena.Humanoids:GetChildren()) do
								local localfacing = lplr.Character.HumanoidRootPart.CFrame.lookVector
								local vec = (v.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).unit
				...
				BindToStepped("Speed", 1, function(time, delta)
					if entity.isAlive then
						...
						elseif speedmethod["Value"] == "WalkSpeed" then 
							...
							lplr.Character.Humanoid.WalkSpeed = speedval["Value"]
						end
			...
			if callback then
				...
				if entity.isAlive then
					...
					for i,v in pairs(enums) do
						lplr.Character.Humanoid:SetStateEnabled(v, false)
					end
					lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Swimming)
					BindToStepped("Swim", 1, function()
						lplr.Character.HumanoidRootPart.Velocity = ((lplr.Character.Humanoid.MoveDirection ~= Vector3.new(0, 0, 0) or lplr.Character.Humanoid.Jump) and lplr.Character.HumanoidRootPart.Velocity or Vector3.new(0, 0, 0))
					end)
				end
			else 
				...
				if entity.isAlive then
					...
					table.remove(enums, table.find(enums, Enum.HumanoidStateType.None))
					for i,v in pairs(enums) do
						lplr.Character.Humanoid:SetStateEnabled(v, true)
					end
				end
			end
		end
